...
4. **O que já virou configuração:** `property_id`, slug, caminho, nome, preço inicial, moeda, aba de destino e identificadores dos CTAs.
...
8. **Tracking já multiempreendimento:** `page_view`, `view_item`, `form_start`, `click_cta`, `view_plants`, `click_whatsapp`, `generate_lead` e o `schedule_visit` legado carregam `property_id`; a planilha roteia cada empreendimento para a aba configurada e a CAPI usa o mesmo identificador.
