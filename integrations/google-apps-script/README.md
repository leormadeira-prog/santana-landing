...
A relação com a região, o perfil sugerido e o critério automático são acrescentados ao final dos cabeçalhos existentes, sem deslocar campos personalizados. A aba **Leads Meta Gamboas** replica os cabeçalhos reais da aba manual, inclusive campos como `Valor Finan. Pré Aprov.`, e acrescenta: email, IDs e nomes de formulário/campanha/conjunto/anúncio, plataforma, indicador orgânico e data da importação. A criação e as sincronizações não movem, alteram nem apagam as linhas manuais. IDs que já existam em qualquer uma das duas abas são tratados como conhecidos para evitar duplicidade.
...
2. executar `setup()` e conferir a extensão dos cabeçalhos de integração até **AQ**, além dos três cabeçalhos de qualificação acrescentados ao final;
...
7. confirme a relação com a região, o perfil sugerido e o critério automático nos cabeçalhos correspondentes do sobrado;
